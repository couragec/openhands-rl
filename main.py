...
规则：
1. **追加**，不要覆盖已有内容
2. 必须按 Iteration 递增写入：读取 summary.md 中最后一个迭代号，当前必须是 N+1；若不满足先修正再写
3. 分析 train.py 源码，提取训练类型和超参数
4. 如果训练失败，从错误日志中提取根因
5. "做了什么"和"为什么"是最重要的字段，必须深入分析
6. 长代码片段每轮最多 1 段，最多 40 行；优先贴本轮新增或改动处
7. 若与上一轮根因相同，避免整段重复，明确写出“新增证据/新增尝试/无新增”
8. 写入后自检 section 完整性：上述字段必须齐全，缺失则补全后再结束
9. 完成后调用 finish 工具结束
""",
    trigger=None,
)
